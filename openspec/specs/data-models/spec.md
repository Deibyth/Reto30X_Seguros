...
Twelve SQLAlchemy 2.0 ORM entities covering all 8 business domains, using async engine (`aiosqlite`) and `Mapped`/`mapped_column` declarative style. Models are auto-created via `Base.metadata.create_all()` on application startup (Alembic deferred to post-MVP).
